...
export default function SearchResult({data}: {data: GameData}) {
    ...
    const button_class = "flex text-6xl self-stretch items-center w-10 justify-center cursor-pointer rounded-tr-xl rounded-br-xl"

    return (
        <div className='flex content-stretch h-full bg-red-300'>
            <div className='flex flex-1 flex-col sm:flex-row'>
                <Link 
                    className='flex flex-1 bg-slate-700 hover:bg-slate-600 h-full content-stretch rounded-tl-xl rounded-bl-xl items-center overflow-clip'
                    href={itad_url ?? ""} 
                >
                    <Image src={banner_url ?? missingImage} width={300} height={140} alt='A banner image of a video game'/>
                    <h3 className="flex-1 text-3xl m-4 font-bold">{title}</h3>
                </Link>
                <Link href={store_url ?? ""} className='flex p-4 flex-col bg-slate-800 hover:bg-slate-700 w-30 justify-center text-center'>
